# Build and Run Script for Workflow Service
# This script stops existing service, builds, and runs workflow-service with cloud Keycloak configuration
...
Write-Host "BUILD AND RUN WORKFLOW SERVICE" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# Kill processes on port 8090 BEFORE anything else
if (-not $SkipStop) {
    Write-Host "[KILL] Killing any processes on port 8090..." -ForegroundColor Yellow
    ...
    for ($i = 1; $i -le 3; $i++) {
        $killed = Stop-ProcessOnPort -Port 8090
        if ($killed) {
    ...
    $finalCheck = Get-NetTCPConnection -LocalPort 8090 -ErrorAction SilentlyContinue | Where-Object {$_.State -eq "Listen"}
    
    if ($finalCheck) {
        Write-Host "  [WARN] Port still in use, force killing all Java processes on port 8090..." -ForegroundColor Yellow
        # Last resort: kill all Java processes using port 8090
    ...
    $finalCheck = Get-NetTCPConnection -LocalPort 8090 -ErrorAction SilentlyContinue | Where-Object {$_.State -eq "Listen"}
    if (-not $finalCheck) {
        Write-Host "  [OK] Port 8090 is now free" -ForegroundColor Green
    } else {
        Write-Host "  [ERROR] Port 8090 still in use! Please manually kill processes." -ForegroundColor Red
...
# Build service
if (-not $SkipBuild) {
    Write-Host "[BUILD] Building workflow-service..." -ForegroundColor Yellow
    try {
...
Write-Host "[FINAL KILL] Ensuring port 8090 is free before starting..." -ForegroundColor Yellow
for ($k = 1; $k -le 5; $k++) {
...
$finalCheck = Get-NetTCPConnection -LocalPort 8090 -ErrorAction SilentlyContinue | Where-Object {$_.State -eq "Listen"}
if ($finalCheck) {
    Write-Host "  [WARN] Port 8090 still in use, force killing ALL Java processes..." -ForegroundColor Yellow
    $pids = $finalCheck | Select-Object -ExpandProperty OwningProcess -Unique
    foreach ($pid in $pids) {
        try {
    ...
    # Kill ALL Java processes using port 8090
    Get-Process -Name "java" -ErrorAction SilentlyContinue | Where-Object {
        $conns = Get-NetTCPConnection -OwningProcess $_.Id -ErrorAction SilentlyContinue | Where-Object {$_.LocalPort -eq 8090 -and $_.State -eq "Listen"}
        $conns
    } | ForEach-Object {
        Write-Host "    Force killing Java process $($_.Id) on port 8090..." -ForegroundColor Yellow
    ...
    # Final verification
    $finalCheck2 = Get-NetTCPConnection -LocalPort 8090 -ErrorAction SilentlyContinue | Where-Object {$_.State -eq "Listen"}
    if ($finalCheck2) {
        Write-Host "  [ERROR] Port 8090 STILL in use! PIDs: $($finalCheck2.OwningProcess -join ', ')" -ForegroundColor Red
...
# Start service
Write-Host "[START] Starting workflow-service on port 8090..." -ForegroundColor Yellow
Write-Host ""

$workflowEnvString = "`$env:KEYCLOAK_BASE_URL='$env:KEYCLOAK_BASE_URL'; `$env:KEYCLOAK_SERVER_URL='$env:KEYCLOAK_SERVER_URL'; `$env:KEYCLOAK_REALM='$env:KEYCLOAK_REALM'; `$env:KEYCLOAK_ADMIN_USERNAME='$env:KEYCLOAK_ADMIN_USERNAME'; `$env:KEYCLOAK_ADMIN_PASSWORD='$env:KEYCLOAK_ADMIN_PASSWORD'; `$env:SPRING_DATASOURCE_URL='$env:SPRING_DATASOURCE_URL'; `$env:SPRING_DATASOURCE_USERNAME='$env:SPRING_DATASOURCE_USERNAME'; `$env:SPRING_DATASOURCE_PASSWORD='$env:SPRING_DATASOURCE_PASSWORD'; `$env:TENANT_SERVICE_URL='$env:TENANT_SERVICE_URL'"
...
Write-Host "[SUCCESS] Workflow service is starting!" -ForegroundColor Green
Write-Host ""
Write-Host "Service Endpoints:" -ForegroundColor Cyan
Write-Host "  Health: http://localhost:8090/actuator/health" -ForegroundColor White
Write-Host "  Camunda: http://localhost:8090/camunda" -ForegroundColor White
Write-Host ""
...
Write-Host ""
if ($serviceReady) {
    Write-Host "[SUCCESS] Workflow service is ready!" -ForegroundColor Green
} else {
